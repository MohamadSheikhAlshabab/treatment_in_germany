...
function tevily_init_scripts(){
	...
	wp_enqueue_script('wow', TEVILY_THEME_URL . '/assets/js/wow/wow.min.js');
	wp_enqueue_script('easing', TEVILY_THEME_URL . '/assets/js/easing/easing.min.js');
	wp_enqueue_script('owlcarousel', TEVILY_THEME_URL . '/assets/js/owlcarousel/owl.carousel.min.js');
	wp_enqueue_script('moment', TEVILY_THEME_URL . '/assets/js/tempusdominus/js/moment.min.js');
	wp_enqueue_script('moment-timezone', TEVILY_THEME_URL . '/assets/js/tempusdominus/js/moment-timezone.min.js');
	wp_enqueue_script('tempusdominus', TEVILY_THEME_URL . '/assets/js/tempusdominus/js/tempusdominus-bootstrap-4.min.js');
	...
	wp_enqueue_style('animate', TEVILY_THEME_URL . '/assets/js/animate/animate.min.css');
	wp_enqueue_style('owlcarousel', TEVILY_THEME_URL . '/assets/js/owlcarousel/assets/owl.carousel.min.css');
	wp_enqueue_style('tempusdominus', TEVILY_THEME_URL . '/assets/js/tempusdominus/css/tempusdominus-bootstrap-4.min.css');
	...
	wp_enqueue_style('style', TEVILY_THEME_URL . '/assets/css/style.css');
...
function contact_us_custom_post_type() {
    register_post_type( 'contact_us', array(
            'labels' => array(
                'name' => __( 'Contact Us' ),
                'singular_name' => __( 'contact_us' )
            ),
            'public' => true,
            'has_archive' => true,
...
add_action( 'init', 'contact_us_custom_post_type' );
...
function enqueue_wowjs() {
    // Enqueue Animate.css
    wp_enqueue_style('animate-css', 'https://cdnjs.cloudflare.com/ajax/libs/animate.css/4.1.1/animate.min.css');

    // Enqueue Wow.js
    wp_enqueue_script('wow-js', 'https://cdnjs.cloudflare.com/ajax/libs/wow/1.1.2/wow.min.js', array('jquery'), '', true);
}
add_action('wp_enqueue_scripts', 'enqueue_wowjs');


function enqueue_tempus_dominus() {
    // Enqueue Bootstrap CSS
    wp_enqueue_style('bootstrap-css', 'https://maxcdn.bootstrapcdn.com/bootstrap/4.5.2/css/bootstrap.min.css');

    // Enqueue Tempus Dominus CSS
    wp_enqueue_style('tempus-dominus-css', 'https://cdnjs.cloudflare.com/ajax/libs/tempusdominus-bootstrap-4/5.39.0/css/tempusdominus-bootstrap-4.min.css');

    // Enqueue Moment.js
    wp_enqueue_script('moment-js', 'https://cdnjs.cloudflare.com/ajax/libs/moment.js/2.29.1/moment.min.js', array('jquery'), '', true);

    // Enqueue Tempus Dominus JS
    wp_enqueue_script('tempus-dominus-js', 'https://cdnjs.cloudflare.com/ajax/libs/tempusdominus-bootstrap-4/5.39.0/js/tempusdominus-bootstrap-4.min.js', array('jquery', 'moment-js'), '', true);
}
add_action('wp_enqueue_scripts', 'enqueue_tempus_dominus');
...
function enqueue_carousel() {
    // Enqueue Owl Carousel CSS
    wp_enqueue_style('carousel-css',TEVILY_THEME_URL . '/assets/css/carousel.css');
	
    // Enqueue Owl Carousel JS
    wp_enqueue_script('carousel-js', TEVILY_THEME_URL .'/assets/js/carousel.js', array('jquery'), '', true);
}
add_action('wp_enqueue_scripts', 'enqueue_carousel');